...
class UsernameValidatorTest {
  ...
  void allowsValidUsernameWhenRegexEnabled() {
    ...
    validator.validateUsername(event);

    assertFalse(event.isCancelled());
    assertTrue(event.getCancelReason().isEmpty());
  }
  ...
  void rejectsInvalidUsernameWhenRegexEnabled() {
    ...
    assertEquals("Username must match [A-Za-z0-9_]{3,16}", event.getCancelReason().orElseThrow());
  }
  ...
  void doesNotProcessAlreadyCancelledEvent() {
    ...
    UsernameValidator validator = new UsernameValidator(config);
    QueueTestUtils.TestPreLoginEvent event = QueueTestUtils.preLoginEvent("Invalid@User!");
    event.setCancelled("Already cancelled");

    validator.validateUsername(event);

    assertTrue(event.isCancelled());
    assertEquals("Already cancelled", event.getCancelReason().orElseThrow());
  }
  ...
  void replacesRegexPlaceholderInMessage() {
    ...
    assertTrue(event.isCancelled());
    assertEquals("Username must match pattern: [A-Za-z]{5,}", event.getCancelReason().orElseThrow());
  }
}
